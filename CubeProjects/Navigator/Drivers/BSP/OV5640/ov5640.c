...
static uint8_t ov5640_focus_init_limited(uint16_t max_steps)
{
    uint16_t i;
    uint8_t state;
    ...
    for (i = 0U; i < (sizeof(ov5640_af_config) / sizeof(ov5640_af_config[0])); i++)
    {
        ov5640_write_reg(0x8000U + i, ov5640_af_config[i]);
    }
...
static uint8_t ov5640_focus_single_limited(uint16_t max_steps)
{
    uint8_t state;
    uint16_t i;
...
uint8_t ov5640_focus_read_status(uint8_t *st3029, uint8_t *st3023, uint8_t *cmd3022)
{
    if (st3029 != NULL)
    {
        *st3029 = ov5640_read_reg(0x3029);
    }
    if (st3023 != NULL)
    {
        *st3023 = ov5640_read_reg(0x3023);
    }
    if (cmd3022 != NULL)
    {
        *cmd3022 = ov5640_read_reg(0x3022);
    }
    return 0U;
}

#define OV5640_VCM_REG0  0x3602U
#define OV5640_VCM_REG1  0x3603U
#define OV5640_VCM_PROBE_AF_WAIT  120U   /* 120*5ms，probe 勿长时间阻塞 */

static uint8_t ov5640_af_wait_fw_status(uint8_t target, uint16_t max_steps)
{
    uint16_t i;
    uint8_t state;
...
static uint8_t ov5640_af_wait_cmd_ack(uint16_t max_steps)
{
    uint16_t i;
    uint8_t ack;
...
static uint8_t ov5640_focus_get_position(uint16_t *pos)
{
    if (pos == NULL)
    {
        return 1U;
    ...
    ov5640_write_reg(0x3022, 0x07);
    if (ov5640_af_wait_cmd_ack(200U) != 0U)
    {
        return 1U;
    }

    *pos = (uint16_t)ov5640_read_reg(0x3024);
    *pos |= (uint16_t)((uint16_t)ov5640_read_reg(0x3025) << 8);
    return 0U;
}

static void ov5640_vcm_manual_set(uint16_t vcm_value)
{
    uint8_t r0;
    uint8_t r1;

    vcm_value &= 0x03FFU;
    r0 = ov5640_read_reg(OV5640_VCM_REG0);
    r0 = (uint8_t)((r0 & 0x0FU) | (uint8_t)((vcm_value & 0x0FU) << 4));
    ov5640_write_reg(OV5640_VCM_REG0, r0);

    r1 = ov5640_read_reg(OV5640_VCM_REG1);
    r1 = (uint8_t)((r1 & 0xC0U) | (uint8_t)((vcm_value >> 4) & 0x3FU));
    ov5640_write_reg(OV5640_VCM_REG1, r1);
}

static uint16_t ov5640_vcm_manual_get(void)
{
    uint8_t r0 = ov5640_read_reg(OV5640_VCM_REG0);
    uint8_t r1 = ov5640_read_reg(OV5640_VCM_REG1);

    return (uint16_t)(((uint16_t)(r0 >> 4) & 0x0FU) | ((uint16_t)(r1 & 0x3FU) << 4));
}

static uint16_t ov5640_abs16_diff(uint16_t a, uint16_t b)
{
    return (a >= b) ? (uint16_t)(a - b) : (uint16_t)(b - a);
}

uint8_t ov5640_vcm_probe(ov5640_vcm_probe_t *out)
{
    uint8_t af_rc;
    uint16_t pos;
    ...
    out->chip_id = (uint16_t)((uint16_t)ov5640_read_reg(0x300A) << 8);
    out->chip_id |= ov5640_read_reg(0x300B);
    if (out->chip_id != OV5640_ID)
    {
        out->verdict = OV5640_VCM_UNKNOWN;
        return 0U;
    }

    af_rc = ov5640_focus_init_limited(OV5640_VCM_PROBE_AF_WAIT);
    out->af_fw_init_ok = (af_rc == 0U) ? 1U : 0U;
    out->fw_status = ov5640_read_reg(0x3029);

    if (out->af_fw_init_ok != 0U)
    {
        (void)ov5640_focus_get_position(&out->lens_pos_idle);
    }

    /* 释放 AF，直接写 VCM DAC 寄存器，观察镜头位置是否变化 */
    ov5640_write_reg(0x3022, 0x08);
    (void)ov5640_af_wait_fw_status(0x70U, 400U);
    ov5640_write_reg(0x3000, 0x20);
    HAL_Delay(20);

    ov5640_vcm_manual_set(120U);
    HAL_Delay(250);
    if (ov5640_focus_get_position(&pos) == 0U)
    {
        out->lens_pos_near = pos;
    }
    else
    {
        out->lens_pos_near = ov5640_vcm_manual_get();
    }

    ov5640_vcm_manual_set(880U);
    HAL_Delay(250);
    if (ov5640_focus_get_position(&pos) == 0U)
    {
        out->lens_pos_far = pos;
    }
    else
    {
        out->lens_pos_far = ov5640_vcm_manual_get();
    }
    out->manual_delta = ov5640_abs16_diff(out->lens_pos_near, out->lens_pos_far);

    /* 恢复 AF 固件后再测单次对焦 */
    af_rc = ov5640_focus_init_limited(OV5640_VCM_PROBE_AF_WAIT);
    if (af_rc == 0U)
    {
        out->af_fw_init_ok = 1U;
        out->fw_status = ov5640_read_reg(0x3029);
        af_rc = ov5640_focus_single_limited(OV5640_VCM_PROBE_AF_WAIT);
        out->af_single_ok = (af_rc == 0U) ? 1U : 0U;
        (void)ov5640_focus_get_position(&out->lens_pos_after_af);
        out->af_delta = ov5640_abs16_diff(out->lens_pos_idle, out->lens_pos_after_af);
    }
    else
    {
        out->af_fw_init_ok = 0U;
        out->af_single_ok = 0U;
    }

    if (out->af_fw_init_ok == 0U)
    {
        out->verdict = OV5640_VCM_ABSENT;
    }
    else if ((out->manual_delta >= 25U) || (out->af_delta >= 15U) || (out->af_single_ok != 0U))
    {
        out->verdict = OV5640_VCM_PRESENT;
    }
    else if ((out->manual_delta < 8U) && (out->af_delta < 8U) && (out->af_single_ok == 0U))
    {
        out->verdict = OV5640_VCM_ABSENT;
    }
    else
...
int ov5640_vcm_probe_format(const ov5640_vcm_probe_t *r, char *buf, unsigned buf_sz)
{
    const char *verdict_str;

    if ((r == NULL) || (buf == NULL) || (buf_sz == 0U))
    {
        return 0;
    }
    ...
    switch (r->verdict)
    {
    case OV5640_VCM_PRESENT:
        verdict_str = "HAS VCM";
        break;
    case OV5640_VCM_ABSENT:
        verdict_str = "NO VCM";
        break;
    default:
    ...
    return snprintf(buf, buf_sz,
                    "OV5640 VCM: %s ID=0x%04X AFinit=%u AFsingle=%u R29=0x%02X "
                    "pos idle/near/far/af=%u/%u/%u/%u dMan=%u dAF=%u",
                    verdict_str,
                    (unsigned)r->chip_id,
                    (unsigned)r->af_fw_init_ok,
                    (unsigned)r->af_single_ok,
                    (unsigned)r->fw_status,
                    (unsigned)r->lens_pos_idle,
                    (unsigned)r->lens_pos_near,
                    (unsigned)r->lens_pos_far,
                    (unsigned)r->lens_pos_after_af,
                    (unsigned)r->manual_delta,
                    (unsigned)r->af_delta);
}
