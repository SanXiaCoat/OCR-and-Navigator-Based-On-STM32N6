...
void ov5640_dcmipp_start(void); /* 启动OV5640 DCMIPP连续传输 */
void ov5640_dcmipp_stop(void);  /* 停止OV5640 DCMIPP传输 */
void ov5640_dcmipp_frame_event(void); /* 帧完成通知（main 回调里调用） */
uint8_t ov5640_dcmipp_snap(uint32_t timeout_ms); /* 单次快照，数据在 ov5640_dcmipp_buf */
