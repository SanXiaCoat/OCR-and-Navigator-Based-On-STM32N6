...
/** 读取 AF 状态寄存器；返回值 0=读成功 */
uint8_t ov5640_focus_read_status(uint8_t *st3029, uint8_t *st3023, uint8_t *cmd3022);

/** VCM 检测结果 */
typedef enum
{
    OV5640_VCM_UNKNOWN = 0,
    OV5640_VCM_PRESENT = 1,
    OV5640_VCM_ABSENT  = 2,
} ov5640_vcm_verdict_t;

typedef struct
{
    uint16_t chip_id;
    uint8_t  af_fw_init_ok;
    uint8_t  af_single_ok;
    uint8_t  fw_status;       /* 0x3029，0x70=AF 空闲 */
    uint16_t lens_pos_idle;
    uint16_t lens_pos_near;
    uint16_t lens_pos_far;
    uint16_t lens_pos_after_af;
    uint16_t manual_delta;
    uint16_t af_delta;
    ov5640_vcm_verdict_t verdict;
} ov5640_vcm_probe_t;

/**
 * @brief 检测 OV5640 模组是否带 VCM（音圈马达）自动对焦
 * @note  需先 ov5640_init()；会短暂驱动 AF 固件与 0x3602/0x3603 手动 VCM
 * @retval 0=检测完成（看 out->verdict）；1=参数无效
 */
uint8_t ov5640_vcm_probe(ov5640_vcm_probe_t *out);

/** 将检测结果格式化为可读字符串（ASCII） */
