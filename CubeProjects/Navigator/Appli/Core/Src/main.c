...
/* 0=导航 UART 显示模式；1=恢复 OCR 检测 */
#define APP_OCR_ENABLE  0
...
int main(void)
{
  ...
  char status_msg[64];
#if APP_OCR_ENABLE
  ocr_det_result_t ocr_res;
  static char status_hint[] = "KEY0:OCR KEY1:FX KEY2:UART4";
#else
  static char status_hint[] = "KEY2:UART4 echo";
#endif
  ...
  LED0(1);
  {
    /* 仅 UART：此时尚未 rgblcd_init / LTDC / HyperRAM，不可写 LCD */
    int uart_early = uart_port_init();

#if !APP_OCR_ENABLE
    /* 导航模式无 DCMIPP：可启用板载 USB-CH340(USART1) 输出日志 */
    uart_init(115200U);
#endif

    if (uart_early == 0)
    {
      uart_port_boot_banner();
      (void)uart_test_start();
    }
    else if (uart_ch340_ready() != 0U)
    {
      uart_ch340_write((const uint8_t *)
          "\r\n=== AI_Vision BOOT (USB only) ===\r\nUART4 init failed\r\n", 52U);
    }
  }
  ...
  uart_port_reinit();
  if (uart_port_is_ready() != 0)
  {
    (void)uart_port_tx_str("=== System ready USB+UART4 115200 ===\r\n");
  }

  if (uart_port_is_ready() != 0)
  {
#if !APP_OCR_ENABLE
    rgblcd_show_string(30, 90, 420, 16, 16, "Log: USB or UART4 115200", YELLOW);
#else
    rgblcd_show_string(30, 90, 420, 16, 16, "UART4 PC10/11 NOT USB", YELLOW);
#endif
  }
  else
  {
    char ue[28];
    (void)snprintf(ue, sizeof(ue), "UART4 init fail");
    rgblcd_show_string(30, 90, 420, 16, 16, ue, RED);
  }
...
#else
  rgblcd_show_string(30, 70, 200, 16, 16, "UART Nav Display", RED);
#endif
  ...
  if (uart_port_is_ready() != 0)
  {
    ...
    {
      char uid_hint[32];
      (void)snprintf(uid_hint, sizeof(uid_hint), "UID:%08lX", (unsigned long)HAL_GetUIDw0());
      ocr_display_status_show2(status_hint, uid_hint, BLUE);
    }
  }
  else if (uart_port_init() == 0)
  {
    (void)uart_test_start();
    nav_uart_init();
    mcu_uid_print_uart();
  ...
  {
    ocr_display_status_show2(status_hint, "UART4 init fail", RED);
  }
#else
...

  {
    ...
    else
    {
      ...
      (void)snprintf(uart_err, sizeof(uart_err), "UART4 init %d", uart_rc);
  ...
  /* USER CODE BEGIN WHILE */
  {
    ...
    nav_uart_poll();
    {
      ...
      if ((uart_port_is_ready() != 0) && ((now - s_hb_ms) >= 10000U))
      {
        s_hb_ms = now;
        (void)uart_port_tx_str("[heartbeat]\r\n");
      }
    }
#endif
    key = key_scan(0);
      ...
      switch (key)
      {
        ...
        case KEY0_PRES:
        {
          ...
          ocr_display_status_show("\xE8\xAF\x86\xE5\x88\xAB\xE4\xB8\xAD...", BLUE);
          {
            ...
            ocr_uart_dump_det_report(cap_rc, &ocr_res, img_width, img_height);
            if (cap_rc == 0)
            {
              ocr_display_focus_poll();
...
#endif /* APP_OCR_ENABLE */
        case KEY2_PRES:
        {
          uint32_t n;

#if !APP_OCR_ENABLE
          ocr_display_status_show2("UART4 echo 30s", "PC10/11 115200", BLUE);
          n = uart_test_echo_session(30000U);
          (void)snprintf(status_msg, sizeof(status_msg), "UART echo %lu B", (unsigned long)n);
          ocr_display_status_show2("UART4 done", status_msg, BLUE);
          nav_uart_redraw();
#else
          ov5640_dcmipp_stop();
          ocr_display_status_show2("UART4 echo 30s", "PC10/11 115200", BLUE);
...
void HAL_DCMIPP_PIPE_FrameEventCallback(DCMIPP_HandleTypeDef *hdcmipp, uint32_t Pipe)
{
  ...
  if (hdcmipp->Instance == DCMIPP)
  {
    if (Pipe == DCMIPP_PIPE0)
    {
      ...
      if (ocr_pipeline_preview_hold_get() == 0U)
      {
        rgblcd_color_fill(img_x_offset, img_y_offset, img_x_offset + img_width - 1,
