...
#if OCR_REC_ENABLE
  if ((s_rec_weights_loaded != 0U) && (address >= OCR_REC_WEIGHTS_XSPI2_BASE) &&
      (address < (OCR_REC_WEIGHTS_XSPI2_BASE + OCR_REC_WEIGHTS_BYTES)))
  {
    return address - OCR_REC_WEIGHTS_XSPI2_BASE + OCR_REC_WGT_RAM_BASE;
  }
#endif
...
uint8_t g_ocr_rec_wgt_pool[OCR_REC_WEIGHTS_BYTES]
    __attribute__((section(".ocr_wgt"), aligned(32)));

static uint8_t s_rec_weights_loaded;

int ocr_rec_weights_load_to_hyperram(void)
{
  uint32_t nor_cpu = OCR_REC_WEIGHTS_XSPI2_BASE;
  uint32_t dst = (uint32_t)(uintptr_t)g_ocr_rec_wgt_pool;
  uint8_t rc;
  uint8_t head[4];

  if (s_rec_weights_loaded != 0U)
  {
    return 0;
  }

  rc = xspi2_nor_read_cpu(nor_cpu, head, sizeof(head));
  if (rc != 0U)
  {
    return -1;
  }
  if ((head[0] == 0xFFU) && (head[1] == 0xFFU) && (head[2] == 0xFFU) && (head[3] == 0xFFU))
  {
    return -2;
  }

  rc = xspi2_nor_read_bulk(nor_cpu, (void *)(uintptr_t)dst, OCR_REC_WEIGHTS_BYTES, NULL);
  if (rc != 0U)
  {
    return -1;
  }

  SCB_InvalidateDCache_by_Addr((uint32_t *)(uintptr_t)g_ocr_rec_wgt_pool,
                               (int32_t)OCR_REC_WEIGHTS_BYTES);
  s_rec_weights_loaded = 1U;
  return 0;
}
