...
void MX_X_CUBE_AI_Process(void)
{
  ...
  const LL_Buffer_InfoTypeDef * ibuffersInfos = NN_Interface_ocr_det.input_buffers_info();
  const LL_Buffer_InfoTypeDef * obuffersInfos = NN_Interface_ocr_det.output_buffers_info();
  ...
  for (int inferenceNb = 0; inferenceNb<10; ++inferenceNb) {
    ...
    LL_ATON_RT_Init_Network(&NN_Instance_ocr_det);  // Initialize passed network instance object
    do {
      /* Execute first/next step */
      ll_aton_rt_ret = LL_ATON_RT_RunEpochBlock(&NN_Instance_ocr_det);
      /* Wait for next event */
